...
return 1;};var _convertPowerOfTwoImage=function _convertPowerOfTwoImage(image){if(!_isImagePowerOfTwo(image)){var canvas=document.createElement("canvas");canvas.width=calcNextPowerOfTwo(image.width);canvas.height=calcNextPowerOfTwo(image.height);var context2d=canvas.getContext("2d");context2d.drawImage(image,0,0,image.width,image.height);image=canvas;}
return image;};var _loadBinFile=function _loadBinFile(url,onload,onerror){var xhr=new XMLHttpRequest();xhr.open('GET',url,true);xhr.responseType="arraybuffer";xhr.onload=function(){onload(xhr.response);};xhr.onerror=function(){if(!(typeof onerror==="undefined"))onerror('not found',url);};xhr.send(null);};var _loadResource=function _loadResource(path,onload,onerror){var extindex=path.lastIndexOf(".");var ext=extindex>=0?path.slice(extindex):"";if(ext==".png"||ext==".jpg"){(function(){var image=new Image();image.onload=function(){var converted_image=_convertPowerOfTwoImage(image);onload(converted_image);};image.onerror=function(){if(!(typeof onerror==="undefined"))onerror('not found',path);};image.crossOrigin="anonymous";image.src=path;})();}else{_loadBinFile(path,function(buffer){onload(buffer);},onerror);}};var loadingEffect=null;var ContextStates=function(){function ContextStates(gl){_classCallCheck(this,ContextStates);this.gl=gl;this.ext_vao=null;this.effekseer_vao=null;this.current_vao=null;this.current_vbo=null;this.current_ibo=null;this.ext_vao=gl.getExtension('OES_vertex_array_object');if(this.ext_vao!=null){this.effekseer_vao=this.ext_vao.createVertexArrayOES();}}
_createClass(ContextStates,[{key:"save",value:function save(){this.current_vbo=this.gl.getParameter(this.gl.ARRAY_BUFFER_BINDING);this.current_ibo=this.gl.getParameter(this.gl.ELEMENT_ARRAY_BUFFER_BINDING);if(this.ext_vao!=null){this.current_vao=this.gl.getParameter(this.ext_vao.VERTEX_ARRAY_BINDING_OES);this.ext_vao.bindVertexArrayOES(this.effekseer_vao);}}},{key:"restore",value:function restore(){if(this.ext_vao!=null){this.ext_vao.bindVertexArrayOES(this.current_vao);}
...
},{key:"loadEffect",value:function loadEffect(path){var scale=arguments.length<=1||arguments[1]===undefined?1.0:arguments[1];var _this=this;var onload=arguments[2];var onerror=arguments[3];this._makeContextCurrent();var effect=new EffekseerEffect(this);var dirIndex=path.lastIndexOf("/");if(typeof scale==="function"){console.log("Error : second arguments is number from version 1.5");effect.scale=1.0;effect.onload=scale;effect.onerror=onload;}else{effect.scale=scale;effect.onload=onload;effect.onerror=onerror;}
...
},{key:"loadEffect",value:function loadEffect(path){var scale=arguments.length<=1||arguments[1]===undefined?1.0:arguments[1];var onload=arguments[2];var onerror=arguments[3];console.warn('deprecated : please use through createContext.');return this.defaultContext.loadEffect(path,scale,onload,onerror);}
