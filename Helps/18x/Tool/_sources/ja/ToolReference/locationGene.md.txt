...
アニメーション付きのFBX（`.fbx`）を読み込むこともできます。FBXファイルで先頭に設定されているアニメーションが再生され、親パーティクルの時間に合わせて進行します。アニメーションで変形したモデルの表面からパーティクルが生成されます。

プロシージャルモデルを使用する場合、プロシージャルモデルのパネルでモデルを作成し、それを指定します。

外部モデルを使用する場合、ツールでは[振る舞い](behavior.md)で登録したモデルを参照します。
実行環境では再生時に外部モデルを指定し、そのインデックスと座標系(ローカル/ワールド)を参照します。
