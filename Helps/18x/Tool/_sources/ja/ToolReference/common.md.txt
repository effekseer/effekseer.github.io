...
生成方法を`連続`または`トリガー`から選択します。
`連続`では一定の周期で継続的に生成し、`トリガー`では指定したトリガーが発火したときだけ生成します。

### 生成時間

パーティクルを生成する周期を設定します。1つ生成されてから次を生成するまでに、指定したフレーム数だけ待機します。この値には1未満も指定できるため、1フレーム中に複数のパーティクルを生成することもできます。
生成モードが`連続`の場合に使用されます。

### 生成開始時間

親パーティクルが生成されてから、このノードが最初のパーティクルを生成するまでの待ち時間を指定します。

後述の`生成開始トリガー`を指定している場合は、トリガーが送信されてから最初のパーティクルが生成されるまでの時間になります。

生成開始時間にはマイナスの値も指定できます。マイナスの時間に生成される予定だったパーティクルは、実際には0フレーム目に生成されます。

下の図は、生成時間、生成開始時間、実際に生成されるタイミングの関係を示したものです。(1)は生成時間、(2)は生成開始時間、(3)は時間（フレーム単位）です。生成開始時間の分だけ開始位置がずれ、そこから生成時間ごとにパーティクルが生成されます。ただし、マイナスの時間に相当するパーティクルはすべて0フレーム目にまとめて生成されます。
...
トリガー番号を指定した場合、そのトリガーを送信するまでパーティクルは生成されません。

デフォルトの`無し`では、トリガーを送信しなくても生成が開始されます。
...
トリガー番号を指定した場合、そのトリガーを送信した時点でパーティクル生成を停止します。

デフォルトの`無し`では、トリガーによる生成停止は行われません。

参照: [15.トリガーを送信してエフェクトを変化させよう](../ToolTutorial/15.md)

### 生成トリガー

生成モードが`トリガー`の場合に、1回の生成を発生させるトリガー番号を指定します。

### トリガーあたり生成数

生成モードが`トリガー`の場合に、1回のトリガーで生成するパーティクル数を指定します。

### トリガーで削除

パーティクルを削除するためのトリガー番号を指定します。

トリガー番号を指定した場合、そのトリガーを送信すると、その時点で存在しているパーティクルを削除します。

デフォルトの`無し`では、トリガーによるパーティクル削除は行われません。

また、`描画共通パネル`の`フェードアウト`を`削除後にフェードアウト`に設定すると、トリガー送信後にフェードアウトが行われます。

参照: [15.トリガーを送信してエフェクトを変化させよう](../ToolTutorial/15.md)

利用できるトリガー種別は、`トリガー0-3`に加えて`親消滅時`と`親衝突時`です。
