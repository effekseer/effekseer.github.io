...
Specifies how UVs are arranged and repeated.
...
Repeats the image based on the ribbon length. This is suitable for expressions with repeating patterns, such as chains.
You can specify the repetition interval with `Tile Length`.

#### Tile (By Particle)

Repeats the image for each particle segment. This is suitable for expressions with repeating patterns, such as chains.
You can also configure it so that the edges are not repeated, or so that only the specified loop region is repeated.
