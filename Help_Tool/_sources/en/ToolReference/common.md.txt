...
`Continuous` spawns particles at a fixed interval, while `Trigger` spawns particles only when the specified trigger is fired.
...
Specifies the delay between the parent particle being generated and the first particle spawned by this node.

If `Spawn Start Trigger` below is specified, this becomes the delay between sending the trigger and spawning the first particle.
...
The following figure shows the relationship between Spawn Rate, Initial Delay, and actual spawn timing. (1) is Spawn Rate, (2) is Initial Delay, and (3) is time in frames. The start time is shifted by the Initial Delay value, and particles are then generated at each Spawn Rate interval. However, all particles that would be generated at negative time are generated on frame 0.
...
When Generation Mode is `Trigger`, this specifies the trigger number that performs one spawn.

### Particles per Trigger

When Generation Mode is `Trigger`, specifies how many particles are spawned for each trigger.
...
Available trigger types are `Trigger 0-3`, `On Parent Removed`, and `On Parent Collision`.
